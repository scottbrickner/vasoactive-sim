...
- **Correction (per the user's direct confirmation, Phase 19d, narrows the "vasoactives are NOT high-alert" correction above):** that correction was scoped to vasoactives specifically, and still holds for them — but this phase extends the modeled formulary to real non-vasoactive drugs (fentanyl, dexmedetomidine, diltiazem), and fentanyl (like propofol/midazolam/ketamine, out of scope this phase) genuinely IS high-alert at this institution and DOES require an independent (two-nurse) double-check before programming, at initiation only (never re-required at titration, consistent with the Phase 8a correction above). Dexmedetomidine and diltiazem remain non-high-alert, same as every vasoactive. `data/policy.ts`'s `MEDICATION_VERIFICATION` is now a per-`DrugId` record rather than one global flag, and `CLAUDE.md`'s non-negotiable rules reflect this correction.
- **Pain-scale convention (Phase 19):** CP 4-156 requires titrating analgesics "to a validated pain assessment instrument" without naming one numerically — it defers to a separate, un-embedded "Pain Assessment" policy. The simulator uses the standard clinical Numeric Rating Scale (0–10, lower is better, fentanyl's analgosedation-scenario goal is ≤4) — confirm this matches your unit's actual instrument.
- **RASS target range (Phase 19):** RASS is the standard -5 to +4 scale; the analgosedation scenario's sedation goal is set at -2 to 0 (a commonly-taught light-sedation range) — scenario-authored, not sourced from Attachment B, the same way the flagship's MAP ≥ 65 target has always been scenario-specific rather than a universal number.
- **Fentanyl/dexmedetomidine activation sequencing (Phase 19):** the analgosedation scenario models dexmedetomidine activating once fentanyl reaches its own starting dose (not its maximum) with pain score still above goal — an authored interpretation of CP 4-156's "adequate analgesia before sedation" requirement, not a numeric threshold Attachment B itself specifies.
- **Diltiazem rate-control range (Phase 19):** Attachment B gives no titration target for diltiazem (dose/increment/interval only, no monitored end-point value) — the 60–100 bpm rate-control range in `diltiazemRateControl.ts` is scenario-authored.
