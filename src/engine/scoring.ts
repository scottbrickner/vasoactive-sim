...
export interface Scorecard {
  ...
  /** Non-scored, encouraging notes about self-corrected moments — a cancelled off-order
   *  attempt never touched the patient, so it doesn't affect overallPercent or any
   *  category's status, but it's still worth surfacing positively (CP 4-156 wants nurses
   *  to catch and correct their own mistakes, not just avoid making them). */
...
export function scoreSession(input: ScoringInput): Scorecard {
  ...
  // since the plan's auto-charted entries are real charted assessments. And entries with
  // `outcome === 'off-order'` are excluded too — those are cancelled, never-applied
  // attempts (see `cancelDoseOverride`) that a learner caught and corrected themselves
  // before anything reached the patient; scoring them the same as an applied violation
  // would penalize the exact self-correcting behavior CP 4-156 wants to see. They're
  // surfaced separately as non-scored coaching notes instead — see the `coachingNotes`
  // field below.
  const normalDoseEntries = doseEntries.filter(
    (e) => !e.underBlockOfCharting && !e.autoGeneratedByMultiStep && e.outcome !== 'off-order',
  )
  ...
  // Same `outcome === 'off-order'` exclusion as category 1/2 above — a sequence-violation
  // attempt that got cancelled via the training-mode override panel never reached the
  // infusion, so it shouldn't count against this category either.
  {
    const sequencedOrders = orders.filter((o) => o.sequence > 1)
    const sequenceAttempts = doseEntries.filter((e) => e.violations?.sequenceNotActivated && e.outcome !== 'off-order')
  ...
  // no `doseAction`. Same `outcome === 'off-order'` exclusion as category 1/2 above.
  {
    const weanedOrders = orders.filter((o) => o.weanOrder != null)
    const weanAttempts = log.filter((e) => e.type === 'action' && e.violations?.wrongWeanOrder && e.outcome !== 'off-order')
  ...
  // Cancelled off-order attempts (see `cancelDoseOverride`) never touched the patient and
  // are excluded from every scored category above — but a learner catching and correcting
  // their own mistake is still worth naming, just outside the scored channel. Rewrites
  // cancelDoseOverride's raw "Titrate/Initiate X to Y — off-order." summary into a
  // positive-toned sentence rather than reusing the terse audit-trail phrasing verbatim.
  const coachingNotes = doseEntries
    .filter((e) => e.outcome === 'off-order')
    .map((e) => {
      const withoutSuffix = e.summary.replace(/ — off-order\.?$/, '')
      const attempted = withoutSuffix.replace(/^Initiate\b/, 'attempted initiating').replace(/^Titrate\b/, 'attempted titrating')
      return `Caught before it reached the patient: ${attempted}, flagged off-order, corrected — no harm done.`
    })

  return { categories, overallPercent, strengths, opportunities, coachingNotes }
}
