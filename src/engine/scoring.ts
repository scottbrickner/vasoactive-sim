...
export function scoreSession(input: ScoringInput): Scorecard {
  ...
  // guided titration leap (see runGuidedTitrationLeap) are correct-by-construction, not
  // a demonstrated learner skill, so they're excluded here too — but category 5
  // (documentation cadence) deliberately keeps reading unfiltered doseEntries below,
  // since the leap's auto-charted entries are real charted assessments.
  const normalDoseEntries = doseEntries.filter((e) => !e.underBlockOfCharting && !e.autoGeneratedByLeap)
