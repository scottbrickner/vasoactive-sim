...
export interface LogEntry {
   ...
   * a guided titration leap (see runGuidedTitrationLeap in state/store.ts) rather than
   * an individual learner click. Excluded from adherence (category 1) and interval/
   * increment (category 2) scoring — it's correct-by-construction, not a demonstrated
   * skill — but still counts for documentation-cadence (category 5): it IS a real
   * charted assessment, just batch-created.
   */
  autoGeneratedByLeap?: boolean
}
